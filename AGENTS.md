...
1. **`/tmp` в Termux не работает** — вообще, не только для cookie-файла: временные файлы
   пишут в `$PREFIX/tmp` (например, `/data/data/com.termux/files/usr/tmp`). Харнесс пишет
   куки в `ck_harness.txt` рядом с собой, а в конце прогона **удаляет его сам**.
